...
if (!$sameSubject) {
    $failures[] = 'Subjekt vergeben';
}
printf(
    "  %-34s %s  %d/%d erfolgreich   %s\n",
    'Subjekt vergeben',
    $sameSubject ? 'OK  ' : 'FEHL',
    count(array_filter($subjects, static fn(array $s): bool => $s['ok'])),
    $workers,
...
if (!$revoked) {
    $failures[] = 'Familienwiderruf nach Replay';
}
printf(
    "  %-34s %s  %s\n",
    'Familienwiderruf nach Replay',
    $revoked ? 'OK  ' : 'FEHL',
